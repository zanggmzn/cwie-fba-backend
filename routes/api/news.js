...
filename: function (req, file, callback) {
    let filename = "news-" + Date.now() + '-' + Math.round(Math.random() * 1E9) + path.extname(file.originalname);
...
fileFilter: (req, file, callback) => {
    if (file.mimetype == "application/pdf" || file.mimetype == "image/png" || file.mimetype == "image/jpg" || file.mimetype == "image/jpeg") {
        ...
        callback(null, false);
        return callback(ErrorBadRequest("Only .pdf .png, .jpg and .jpeg format allowed!"));
    }
},
...
router.post(
    "/",
    auth.required,
    upload.single('news_file_upload'),
